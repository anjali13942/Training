CREATE TABLE IF NOT EXISTS information_schema.column_domain_usage (
domain_catalog character varying ,
domain_schema character varying ,
domain_name character varying ,
table_catalog character varying ,
table_schema character varying ,
table_name character varying ,
column_name character varying 
);
GRANT INSERT, SELECT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER on information_schema.column_domain_usage to postgres;
GRANT SELECT on information_schema.column_domain_usage to PUBLIC;
